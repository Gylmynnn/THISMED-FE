...
class Paddings {
  ...
  static Widget customAll(
      Widget child, {double? T, double? B, double? L, double? R}) {
    return Padding(
      padding: EdgeInsets.only(top: T ?? 0, bottom: B ?? 0, left: L ?? 0, right: R ?? 0),
      child: child,
    );
  }

   static Widget customSyx(
      Widget child, {double? sY, double? sX}) {
    return Padding(
